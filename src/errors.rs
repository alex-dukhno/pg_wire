...
mod hand_shake_error {
    ...
    impl<'e> Display for HandShakeError<'e> {
        fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
            match &self.kind {
                HandShakeErrorKind::UnsupportedProtocolVersion(code) => write!(f, "Unsupported Protocol Version: {}", code),
                HandShakeErrorKind::UnsupportedClientRequest(code) => write!(f, "Unsupported Client Code Request: '{}'", code),
    ...
    mod formatting {
        ...
        use crate::request_codes::VERSION_1_CODE;

        #[test]
        fn from_payload_error() {
            assert_eq!(HandShakeError::from(PayloadError::from(PayloadErrorKind::EndOfBuffer)).to_string(), "End of Payload Buffer");
        }

        #[test]
        fn unsupported_protocol_version() {
            assert_eq!(HandShakeError::from(HandShakeErrorKind::UnsupportedProtocolVersion(VERSION_1_CODE)).to_string(), "Unsupported Protocol Version: Version 1.0 Request");
        }

        #[test]
        fn unsupported_client_request() {
            assert_eq!(HandShakeError::from(HandShakeErrorKind::UnsupportedClientRequest(Code(0x12_34_56_78))).to_string(), "Unsupported Client Code Request: 'High bytes 0x1234 Low bytes: 0x5678'");
        }
    }
}

mod message_format_error {
    use crate::errors::{PayloadError, NotSupportedOid, UnrecognizedFormat};
    use std::fmt::{self, Display, Formatter};
    ...
    mod formatting {
        ...
        fn from_not_supported_oid() {
            assert_eq!(MessageFormatError::from(NotSupportedOid(100)).to_string(), "OID: '100' is not supported");
        }

        #[test]
        fn from_unrecognized_format() {
            assert_eq!(MessageFormatError::from(UnrecognizedFormat(5)).to_string(), "unknown format code: 5");
        }

        #[test]
        fn unsupported_frontend_message() {
            assert_eq!(MessageFormatError::from(MessageFormatErrorKind::UnsupportedFrontendMessage('t')).to_string(), "unsupported frontend message tag 't'");
        }

        #[test]
        fn missing_message_tag() {
            assert_eq!(MessageFormatError::from(MessageFormatErrorKind::MissingMessageTag).to_string(), "Message tag is not found in payload");
        }

        #[test]
        fn invalid_type_byte() {
            assert_eq!(MessageFormatError::from(MessageFormatErrorKind::InvalidTypeByte('U')).to_string(), "invalid type byte in Describe frontend message: 'U'");
        }
    }
}
